...
if __name__=='__main__':
    ...
    t0 = time.time()
    rmax = 200
    discard_site = (N-rmax)/2
    string_corr = TNpy.measurement.string_corr(Gs[:],discard_site=discard_site,N_conf=100)
    dist, corr = string_corr.avg_corr()
    print "Execution time = {}".format((time.time()-t0)/60.)+" mins."

    TNpy.data.io.save(os.getcwd()+'/string_corr-N_{}-g_{}-ma_{}-chi_{}'.format(N,g,ma,chi),[dist,corr])    
